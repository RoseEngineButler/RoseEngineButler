...
**GUI layer** (`REB_Display/`): AXIS (`DISPLAY = axis`) with a GladeVCP side panel (`REB_Panel_v2.ui`) and three embedded tabs (Help, Settings, License), all driven by the single handler script `rosetta.py` (loaded once per gladevcp instance via `-u REB_Display/rosetta.py`). Each `.ui` file's Python object IDs (e.g. `REBCnfg.X_Ena_Override`) are what HAL nets in REB_PostGUI.hal connect to — cross-reference the `.ui` files and REB_PostGUI.hal together when tracing a panel control end-to-end. `REB_Display/ZZ_Unused/` holds superseded `.ui` files kept for reference; don't wire new HAL against them.
