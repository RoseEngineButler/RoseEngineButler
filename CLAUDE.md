...
- `RoseEngineButlerLocal/REB_Settings_v1.ini` — persisted per-axis stepgen scale values (an XML file despite the `.ini` extension), written by `REB_Display/REB_Scale_Persist.py` on shutdown and read back by `REB_Display/rosetta.py` at Settings-tab load
